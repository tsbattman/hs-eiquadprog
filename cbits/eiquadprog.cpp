...
double solve_quadprog(const MatrixBase<Matrix> & G,  const MatrixBase<Vector> & g0,
    const MatrixBase<Matrix>& CE, const MatrixBase<Vector>& ce0,
    const MatrixBase<Matrix>& CI, const MatrixBase<Vector>& ci0,
    MatrixBase<Vector>& x)
{
  using std::abs;
  int i, j, k, l; /* indices */
...
char hs_is_solution (double v) {
  return std::numeric_limits<double>::infinity() != v;
}


double hs_solve_quadprog( double* G, double* g0, double* CE, double* ce0,  double* CI, double* ci0, double* x,
    int n, int p, int m) {
  Map<VectorXd> vx(x, n);
